...
class TargetLabels:
    ...
    def __init__(self, dataset_name, n_profiling, n_validation, n_attack, target_byte, leakage_model, file_path):
        ...
        self.y_profiling = np.zeros((16, n_profiling, self.classes))
        self.y_validation = np.zeros((16, n_validation, self.classes))
        self.y_attack = np.zeros((16, n_attack, self.classes))

        self.profiling_labels = np.zeros((16, n_profiling))
        self.validation_labels = np.zeros((16, n_validation))
        self.attack_labels = np.zeros((16, n_attack))

        self.labels_key_hypothesis_validation = np.zeros((16, 256, n_validation), dtype='int64')
        self.labels_key_hypothesis_attack = np.zeros((16, 256, n_attack), dtype='int64')
        ...
        for kb in range(16):
            ...
            if dataset_name == "dpav42":
                if n_validation < 5000:
                    ...
                    self.correct_key_validation[kb] = bytearray.fromhex(self.round_key_validation)[kb]
                    self.correct_key_attack[kb] = bytearray.fromhex(self.round_key_attack)[kb]
                else:
                    self.round_key_validation = "5384FACAAEFD16F38F1359ACE6A29037"
    ...
    def load_dataset(self):
        ...
        for kb in range(16):
            self.profiling_labels[kb] = self.aes_labelize(profiling_plaintexts, profiling_keys, target_byte=kb)
            self.validation_labels[kb] = self.aes_labelize(validation_plaintexts, validation_keys, target_byte=kb)
            self.attack_labels[kb] = self.aes_labelize(attack_plaintexts, attack_keys, target_byte=kb)

            self.y_profiling[kb] = to_categorical(self.profiling_labels[kb], num_classes=self.classes)
            self.y_validation[kb] = to_categorical(self.validation_labels[kb], num_classes=self.classes)
            self.y_attack[kb] = to_categorical(self.attack_labels[kb], num_classes=self.classes)

            if self.dataset_name == "dpav42":
                self.labels_key_hypothesis_validation[kb] = self.create_labels_key_guess(validation_plaintexts,
                                                                                         self.round_key_validation)
                self.labels_key_hypothesis_attack[kb] = self.create_labels_key_guess(attack_plaintexts,
                                                                                     self.round_key_attack)
            else:
                self.labels_key_hypothesis_validation[kb] = self.create_labels_key_guess(validation_plaintexts,
                                                                                         self.round_key)
    ...
    def aes_labelize(self, plaintexts, keys, target_byte=None):
        ...
        plaintext = [row[target_byte] for row in plaintexts]
        key = [row[target_byte] for row in keys]
    ...
    def create_intermediates_ascadf(self, profiling_plaintext, profiling_masks, profiling_key, attack_plaintext,
        ...
        for byte in range(14):
            if self.leakage_model == "HW":
                share1_profiling[byte + 2, :] = [bin(int(iv)).count("1") for iv in profiling_masks[:, byte]]
                share2_profiling[byte + 2, :] = [bin(aes_sbox[int(p) ^ int(k)] ^ int(m)).count("1") for p, k, m in
                                                 zip(profiling_plaintext[:, byte + 2], profiling_key[:, byte + 2],
                                                     profiling_masks[:, byte])]
                share1_attack[byte + 2, :] = [bin(int(iv)).count("1") for iv in attack_masks[:, byte]]
                share2_attack[byte + 2, :] = [bin(aes_sbox[int(p) ^ int(k)] ^ int(m)).count("1") for p, k, m in
        ...
        if self.leakage_model == "HW":
            share2_profiling[0, :] = [bin(aes_sbox[int(p) ^ int(k)]).count("1") for p, k, in
                                      zip(profiling_plaintext[:, 0], profiling_key[:, 0])]
            share2_attack[0, :] = [bin(aes_sbox[int(p) ^ int(k)]).count("1") for p, k in
                                   zip(attack_plaintext[:, 0], attack_key[:, 0])]
            share2_profiling[1, :] = [bin(aes_sbox[int(p) ^ int(k)]).count("1") for p, k, in
                                      zip(profiling_plaintext[:, 1], profiling_key[:, 1])]
            share2_attack[1, :] = [bin(aes_sbox[int(p) ^ int(k)]).count("1") for p, k in
                                   zip(attack_plaintext[:, 1], attack_key[:, 1])]
        else:
